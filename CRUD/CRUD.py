...
class Crud:
    ...
    def select_all2(self, primaryKey_value=None):
        if primaryKey_value is None:
            select_query = sql.SQL("SELECT * FROM {}").format(sql.Identifier(self.table))
            self._execute(select_query)
        else:
        ...
        try:
            ...
            selected2 = {}
            print('-# ' + str(selected) + '\n')
            for i in range(len(selected[0])-1, -1, -1):
                name = str(self._cursor.description[i][0])
                selected2[name] = selected[0][i]

            return selected2

    def select_conditional_org(self, column, value):
        select_query = sql.SQL("SELECT * FROM  {} WHERE {} = 'cytail1' ;").format(
            sql.Identifier(self.table),
    ...
    def delete2(self, column, column_value, primaryKey_value):
        delete_query = sql.SQL("DELETE FROM {} WHERE {}= {} ").format(
            sql.Identifier(self.table),
            sql.Identifier(column),
            sql.Placeholder(),
        )
        self._execute(delete_query, (column_value))
        self._counter += 1
