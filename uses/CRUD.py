...
class Crud:
    ...
    def columns_names2(self):
        self.connect()
        test = "{}".format(sql.Identifier(self.table))
        test = test.replace('"', "'")
        select_query = sql.SQL("SELECT column_name FROM information_schema.columns WHERE table_name = '{}';").format(
            test
        )
        # select_query = sql.SQL(
        #     "SELECT column_name FROM information_schema.columns WHERE table_name = 'organization_profile';")
