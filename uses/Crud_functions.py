...
def modify_asset_value(mapping, mapid):
    asset_mapping.connect()
    asset_mapping.update_multiple_columns(columns=mapping.keys(), columns_value=mapping.values(),
...
def deletechoice(data):
    choice.connect()
    question_vulnerability.connect()
    choices_ids = question_vulnerability.select(columns=['choices', ], primaryKey_value=data['question_id'])[0][0]
    choices_ids.remove(data['choice_id'])
    question_vulnerability.update(column='choices', column_value=choices_ids, primaryKey_value=data['question_id'])
    choice.delete(primaryKey_value=data['choice_id'])
    choice.commit()
    choice.close()
...
def modifychoice(data):
    choice.connect()
    choice.update_multiple_columns(columns=data['choice'].keys().remove('choice_id'),
                                   columns_value=data['choice'].values().remove(
                                       data(['choice']['choice_id'])),
                                   primaryKey_value=data(['choice']['choice_id']))
    choice.commit()
    choice.close()
