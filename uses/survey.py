...
# @app.route('/survey/quizzes/<int:id_quiz>', methods=['GET'])
# def quizzes_id(id_quiz):
#     qu = get_quiz(id_quiz)[0]
#     one_quiz = {'id': qu[0], 'title': qu[1], 'description': qu[2], 'questions': [], 'title_fr': qu[5],
#                 'description_fr': qu[6]}
#     for i in qu[3]:
...
#             choices.append({"id": ch[0], "title": ch[1]})
#         question_model = {'id': i[0], 'title': i[1], 'choices': choices}
#         one_quiz["questions"].append(question_model)
#     return json.dumps(one_quiz, sort_keys=False)
#     # vquestions = sorted(get_all_vquestion(), key=lambda x: x[0])
#     #
#     # dictionnairev = {"Questions related to": "vulnerabilities", "Questions": []}
#     # for i in vquestions:
#     #     choices = []
#     #     for j in i[2]:
#     #         ch = get_choice(int(j))[0]
#     #         choices.append({"id": ch[0], "title": ch[1]})
#     #     question_model = {'id': i[0], 'title': i[1], 'choices': choices}
#     #     dictionnairev["Questions"].append(question_model)
#     # return json.dumps(dictionnairev, sort_keys=False)
#
#
# @app.route('/survey/quizzes', methods=['GET'])
# def quizzes():
#     quiz_list = sorted(get_quizzes(), key=lambda x: x[0])
#     all_quizzes = []
#     for i in quiz_list:
#         single_quiz = {'id': i[0], 'title': i[1], 'description': i[2], 'questions': [], 'title_fr': i[5],
#                        'description_fr': i[6]}
#         for j in i[3]:
#             question = get_question(j)[0]
#             choices = []
#             for k in question[2]:
#                 ch = get_choice(int(k))[0]
#                 choices.append({"id": ch[0], "title": ch[1]})
#             question_model = {'id': j[0], 'title': j[1], 'choices': choices}
#             single_quiz["questions"].append(question_model)
#         all_quizzes.append(single_quiz)
#     return json.dumps(all_quizzes, sort_keys=False)


# @app.route('/survey/oranization_profile/', methods=['GET', 'POST'])
# def profilecreation():
#     schema2 = organizationprofile.organization_names()
#     if request.method == 'GET':
#         requestdata = request.get_json()
#         organizationdata = organizationprofile.select_all(primaryKey_value=requestdata['id'])
...
#                 # making the question in format that will be easy readable from json file ,
#                 # creating list of questions for each use case as the value of the dictionary
#                 organizationprofiles[schema2[i]] = organizationdata[0][i]
#             return json.dumps(organizationprofiles, sort_keys=False)
#         else:
#             return json.jsonify("Profile does not exist")
#     if request.method == 'POST':
#         data = request.get_json()
#         e_profile = {schema2[0]: data['id']}
#         for i in data['answers'].keys():
#             e_profile[schema2[int(i)]] = data['answers'][i]
#         addentrepriseprofile(e_profile)
